...
if (!fs.existsSync(CONFIG_FILE)) {
  fs.writeFileSync(CONFIG_FILE, JSON.stringify({
    ...
    metaTitle: 'روابطي',
    metaTitle_en: 'My Links',
    metaDescription: 'اختر طريقتك المفضلة للتواصل',
    metaDescription_en: 'Choose your preferred way to connect',
    siteLogo: null,
...
const migrateConfig = () => {
  ...
  // Metadata used to share the visible page title and tagline. Copy their
  // existing values once, then let users manage display and sharing text
  // independently. Empty display titles remain empty instead of becoming a
  // generic "My Links" title in browsers and social apps.
  if (current.metaTitle === undefined) {
    migrated.metaTitle = current.siteTitle || '';
    migrated.metaTitle_en = current.siteTitle_en || '';
    migrated.metaDescription = current.siteTagline || '';
    migrated.metaDescription_en = current.siteTagline_en || '';
    changed = true;
  }
